...
function vremya () {
        TIME=15
             echo "Введите данные сторон треугольника"
        read -t $TIME a b c 
     
        if [ -z "$a" ] || [ -z "$b" ] || [ -z "$c" ]; then
	     echo "Время ожидания истекло или не задана одна из сторон"
        exit
        fi
}
...
function triangle () {
        local a=$1 b=$2 c=$3 
	if ! [ -z $(echo $a | tr -d '-' | sed s/[0-9]//g) ] || ! [ -z $(echo $b | tr -d '-' | sed s/[0-9]//g) ] || ! [ -z $(echo $c | tr -d '-' | sed s/[0-9]//g) ]; then	         
	   echo "Неверные данные. Нужно вводить числа" #проверка на ввод именно чисел, а не символов. SED заменяет цифры на пустоту, если значение не пустое, значит введены символы. TR пропускает - для следующего условия 
	exit   
        elif [ $(echo "$a <= 0" | bc) -eq 1 ]  || [ $(echo "$b <= 0" | bc) -eq 1 ] || [ $(echo "$c <= 0" | bc) -eq 1 ]; then
	    echo "Стороны треугольника не могут быть меньше или равны 0"
	exit   
...
function run_test () {
	set -x -e
     ...
     [ "$(math_test 9223372036854775807 9999999999999999 18446744073709551616)" = "Обычный треугольник" ]
}
